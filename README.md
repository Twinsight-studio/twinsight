...
docs/infra/           雲端資源建置 runbook（v1 未使用，保留供日後參考）
```
...
build / Alembic migration / 連線池壓測，通過後 merge，實際部署由你在本機執行上面的指令。
...
> 若日後要改走雲端託管，建置步驟保留在 [`docs/infra/gcp-setup.md`](docs/infra/gcp-setup.md)。
